...
/// Create stateless routes for the Geo module.
///
...
///
/// ```ignore
/// let module = GeoModule::builder().with_database(pool).build()?;
/// let routes = create_stateless_routes(&module);
/// let app = Router::new().merge(routes);
/// ```
pub fn create_stateless_routes(module: &crate::GeoModule) -> Router<()> {
    Router::new()
        .merge(create_city_routes(module.city_service.clone()))
        .merge(create_country_routes(module.country_service.clone()))
...
pub fn get_routes(module: &crate::GeoModule) -> Router<()> {
    Router::new()
...
/// Create stateful routes for the Geo module.
///
...
pub fn create_combined_routes(module: &crate::GeoModule) -> Router<AppState> {
...
/// Get all routes (stateful) for the Geo module.
///
/// This is a convenience function that wraps routes with API versioning.
pub fn get_routes_with_state(module: &crate::GeoModule) -> Router<AppState> {
    Router::new()
